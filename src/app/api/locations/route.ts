...
export async function GET(request:Request){try{await requireSession();const clientId=new URL(request.url).searchParams.get("clientId")||undefined;return Response.json({locations:await prisma.location.findMany({where:{clientId,status:"ACTIVE"},include:{client:{select:{companyName:true}}},orderBy:{name:"asc"}})})}catch(error){return apiError(error)}}
